...
\begin{document}
    \subsection{Valuing unsecured swaps}
        ...
        \\
        Remembering the single-period model, where the probability of the assets being exactly equal to the liability is 0.
        The same argument is assumed in the multi-period model,
        such that $\mathbb{P}\left(A=L+W+L_2+\theta_{1}R_{1}\right) = 0$.
        And with no specifications made about the counterparty's capital structure,
        it's also assumed that there exists a small enough quantity, $q$,
        such that the default indicator is not changing value by obtaining that size of investment.
        By L'H\^{o}pital's % cspell: disable-line
        the numerator then equals 0 in the limit,
