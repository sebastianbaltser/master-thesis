...
\begin{document}
    \subsection{Valuing unsecured swaps}
        ...
        With the face value of the new debt now defined as $D_{0} = U(q)(R_{0} + s_{0}(q))(R_{1} + s_{1}(q))$,
        the marginal increase in the value of the firm's equity per unit investment is therefore:
        \begin{equation}
            G_{\text{debt}} =
            \quad\frac{\partial}{\partial q}
            \mathbb{E}_{0}^{\rnmeasure}\left.\left[
                \delta_{2} \left(
                    A - W + qY_{1} + qY_{2}
